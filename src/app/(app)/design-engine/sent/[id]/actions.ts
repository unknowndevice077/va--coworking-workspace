...
export async function replyToDesignFeedbackAction(formData: FormData) {
  ...
  if (!approval || approval.workspaceId !== user.workspaceId) return;

  await prisma.designComment.create({ data: { workspaceId: user.workspaceId, approvalId, body, fromVA: true } });
